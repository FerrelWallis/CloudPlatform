...
#args <- c("-i","E:/projects/DrawPicture-r/com/work/PCA-/data/table.txt",
#          "-o","C:/Users/Administrator/Desktop/ss","--scale","FALSE"
#)
...
spec <- matrix(c("tablepath", "i", 2, "character", "The path to the table data read", "",
                 ...
                 "scale","sca",1,"logical","scale data","FALSE"
), byrow = TRUE, ncol = 6)
if (!cheackun(spec)) {
...
#-------------------------------------------------------------------------------
setwd(opt$filepath)
da <- read.delim(opt$tablepath,sep="\t",head=T,check.names = FALSE)
type <- c(rep("a",7),rep("b",7))
rownames(da) <- da[,1]
da <- da[,-1]
...
if(opt$scale){
  #画图数据
  write.table(x=pca12,file="pca.scale.x.xls",sep = "\t")
  #标准差
  write.table(x=pvar,file="pca.scale.sdev.xls",sep = "\t")
  #特征向量矩阵
  write.table(x=pca$rotation ,file="pca.scale.rotation.xls",sep = "\t")
}else{
  #画图数据
  write.table(x=pca12,file="pca.x.xls",sep = "\t")
  #标准差
  write.table(x=pvar,file="pca.sdev.xls",sep = "\t")
  #特征向量矩阵
  write.table(x=pca$rotation ,file="pca.rotation.xls",sep = "\t")
}