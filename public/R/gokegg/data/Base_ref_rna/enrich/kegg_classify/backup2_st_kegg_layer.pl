...
while(<D>){
    ...
    for(my $i=0;$i < @b;$i++){
        ...
        my @d=split /\_se/,$c[0];
        $h{$d[0]}=1;
        $all2{$lay1{$lay3{$a[1]}}}{$d[1]}=1;
        $tar2{$lay1{$lay3{$a[1]}}}{$lay2{$lay3{$a[1]}}}{$d[1]}=1;
        $st_gene{$d[1]}++;$st_isof{$c[0]}++;
    }
...
foreach my $k1(sort {$kk{$a}<=>$kk{$b}} keys %kk){
    ...
    foreach my $k2(sort keys %{$tar{$k1}}){
        ...
        if(exists $tar{$k1}{$k2}){
            ...
            print OUT2 "$tar{$k1}{$k2}($num)\n";
            print OUT "$k2";
            printf OUT "\t%d\t%d\t%.4f\t%d\t%.4f\n",$kk{$k1},$tar{$k1}{$k2},$tar{$k1}{$k2}/$num_isof,$num,$num/$num_gene;
        }
        else{
            print OUT2 "0(0)\n";
        }
