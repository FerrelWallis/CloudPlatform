...
for j=1:length(type);
    ...
    for i=1:length(tar1(:,2))
        kk1=text(-0.01,y_start+width/2,test1{i},'HorizontalAlignment','right','Interpreter','none');
...
text(x_max*0.4,-3,'Percent of Isoform(%)','HorizontalAlignment','left','Interpreter','none','fontsize',10);hold on;
...
saveas(gca,'kegg_classification_isoform.pdf');close;
...
for j=1:length(type);
    ...
    for i=1:length(tar1(:,2))
        kk1=text(-0.01,y_start+width/2,test1{i},'HorizontalAlignment','right','Interpreter','none');
        kk2=text(tar1(i,5)+0.02*x_max,y_start+width/2,num2str(tar1(i,4)),'HorizontalAlignment','left','Interpreter','none','fontsize',9); %基因数量
        rectangle('position',[0,y_start,tar1(i,5),width],'facecolor',color1(j,:));hold on;
        y_start=y_start+1;
    end
end
%添加x轴刻度
...
text(x_max*0.4,-3,'Percent of Gene(%)','HorizontalAlignment','left','Interpreter','none','fontsize',10);hold on;
%plot(x_max*ones(1,20),linspace(-jianju,0,20),'k');hold on;
set(gca,'xtick',[],'ytick',[],'xcolor',[254 254 254]/255,'ycolor',[254 254 254]/255);
axis([-0.3*x_total x_total*0.8 -1.2 y_start]);
saveas(gca,'kegg_classification_gene.pdf');close;